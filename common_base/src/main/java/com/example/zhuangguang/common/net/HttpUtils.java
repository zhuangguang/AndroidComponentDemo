...
//        builder.addSSLSocketFactory(NovateHttpsFactroy.creatSSLSocketFactory(BaseApplication.getApplication().getBaseContext(), "xxx.cer"));
//        builder.addHeader(headers); //添加公共请求头
//        builder.addParameters(parameters);//公共参数
//        builder.connectTimeout(10);//连接时间
//        builder.addCache(true);//是否缓存
//        builder.addCache(cache,cacheTime);//自定义缓存
//        builder.addLog(true);//是否开启log
//        builder.cookieManager(new NovateCookieManager());//自定义cooike
...
//        builder.client()
    ...
    private void request(int methodType, String url, RequestParam params, String tag, ResponseCallback callback){
       checkBaseUrl();
        com.orhanobut.logger.Logger.e(baseUrl + url + params.toJson());
        switch (methodType){
            case REQUEST_POST:
